...
NTSTATUS c_driver::send_serivce(ULONG ioctl_code, LPVOID io, DWORD size)
{
	...
	if (!h_driver || h_driver == INVALID_HANDLE_VALUE)
...
PVOID c_driver::alloc_memory_ex(DWORD size, DWORD protect)
{
	...
	if (send_serivce(ioctl_alloc_memory, &req, sizeof(req)) != STATUS_SUCCESS)
		return nullptr;
...
void c_driver::disconnect()
{
	std::lock_guard<std::mutex> lock(m_mutex);
	disconnect_unlocked();
}
void c_driver::disconnect_unlocked()
{
	if (h_driver && h_driver != INVALID_HANDLE_VALUE)
	{
...
bool c_driver::is_loaded() const
{
	std::lock_guard<std::mutex> lock(m_mutex);
	return h_driver && h_driver != INVALID_HANDLE_VALUE;
}
c_driver::~c_driver()
{
	disconnect();
